// Alternative: Flat structure API endpoint
// Vercel should detect this at /api/logs-activity
// Access via: /api/logs-activity (NOT /api/logs/activity)
//
// This is a fallback if the nested folder structure doesn't work
...
module.exports = (req, res) => {
  ...
  const now = Date.now();
  const logs = [
    { timestamp: new Date(now).toISOString(), agent: 'Nexus', type: 'system', message: 'API endpoint /api/logs-activity is working! (FLAT STRUCTURE)', sessionId: 'api-test-flat' },
  ...
  res.status(200).json({
    success: true,
    endpoint: '/api/logs-activity (flat structure)',
